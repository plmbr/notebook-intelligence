...
- **Credentials and model** follow the same env-override locks as Claude: `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `NBI_ACP_CHAT_MODEL` pin and hide the corresponding fields. When NBI supplies the API key, Codex no longer writes the key to disk or passes it into the environment of the commands it runs: the key is held in memory (`cli_auth_credentials_store = "ephemeral"`) rather than in `auth.json`, so a changed key takes effect the next time the agent starts; Codex's shell environment snapshots are turned off (`features.shell_snapshot = false`); and the key's variable is removed from commands' environment (`shell_environment_policy.exclude`). The next time the agent starts, NBI also removes the `auth.json` and shell snapshot files that earlier versions left in its `codex-home` directory. Removing them does not undo earlier exposure, so rotate any key used with ACP mode before this version. What this does not cover: a key typed into the ACP tab is still saved in NBI's `config.json`, which commands running as the same user can read, so prefer supplying it through `OPENAI_API_KEY`; the key stays in the environment of the codex-acp process (and of the Jupyter server, when it comes from `OPENAI_API_KEY`), where a command running outside Codex's sandbox as the same user can read it; a shell startup file that exports the key and that the login shell reads (`~/.zshenv`, `~/.zprofile`, `~/.bash_profile`, `~/.profile`, or a file they source, such as a `~/.bashrc` sourced from `~/.profile`) sets it again for commands; the `exclude` list replaces, rather than adds to, one set in system Codex configuration; and an adapter other than codex-acp 0.16.0 launched through `NBI_ACP_AGENT_COMMAND` may ignore these settings. With snapshots off, Codex no longer runs commands from a snapshot of an interactive shell, so PATH changes, aliases, and environment managers (conda, pyenv, nvm) set up only in `~/.zshrc` or in a `~/.bashrc` that the login shell does not source do not apply to them.
