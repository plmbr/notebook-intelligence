...
| `NBI_ACP_AGENT_COMMAND`                          | str  | unset                       | env                                | Overrides the adapter command NBI launches for the selected ACP agent type (shell-split). For Codex, NBI appends its `-c` approval and sandbox options, so a replacement must pass them through to codex-acp. Unset uses the agent's registered default command. See [Gating the experimental ACP agent](#gating-the-experimental-acp-agent-378).                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- The pins rely on codex honoring the command-line `-c approval_policy` and `-c sandbox_mode` overrides above its user and project configuration. NBI does not parse or strip codex's own config; if a future codex version applies a higher-precedence per-project setting, that assumption would need revisiting.
- A command that matches an allow rule in Codex's command rules runs outside the sandbox. Codex loads rules from its config directory (NBI's isolated `CODEX_HOME` with API-key auth, the user's `~/.codex` with ChatGPT auth), from system and managed configuration, and, on the ChatGPT path, from a trusted project's `.codex/rules`.
- Under full access the writable workspace is the Jupyter server's root directory, and files the agent writes there can run later outside the sandbox without review. In any root that includes packages in a project `.venv` and the Claude project settings and hooks (`.claude/settings.json`, `.mcp.json`) that Claude mode reads when its Project setting source is on. If the root is a home directory or above it, it also includes shell startup files, IPython startup scripts, Jupyter's own configuration, and `~/.jupyter/nbi`, which holds the isolated `CODEX_HOME` config and command rules and NBI's `mcp.json`. Treat full access as letting the agent run code as the Jupyter server's account on a later action, and set the root to a project directory rather than a home directory.
...
- On Windows, Codex reads `managed_config.toml` from `CODEX_HOME`. With API-key auth that is NBI's `codex-home` directory, which the account running Jupyter can write, so on that platform the pins, including the default `untrusted` one, are not tamper-proof against that account.
